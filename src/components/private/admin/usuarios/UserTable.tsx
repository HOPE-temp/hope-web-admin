...
const globalUserFilter: FilterFn<any> = (row, columnId, filterValue) => {
  ...
  return (
    (row.original.username?.toLowerCase().includes(value) ?? false) ||
    (row.original.firstName?.toLowerCase().includes(value) ?? false) ||
    (row.original.lastName?.toLowerCase().includes(value) ?? false) ||
    (row.original.documentNumber?.toLowerCase().includes(value) ?? false)
  );
};

export function UserTable<TData extends RowData>({ data, columns }: UserTableProps<TData>) {
