...
const Calendar = ({
  ...
  function handleClickSchedule(id: string) {}
  ...
  return (
    <div className="h-20 border-t last:border-b">
      {events
        ...
        .map(event => {
          let maxDate: Date;
          const nextDayDate = startOfDay(addDays(event.start, 1));
          if (event.end.getTime() > nextDayDate.getTime()) {
            maxDate = nextDayDate;
          } else {
            maxDate = event.end;
          }
          const hoursDifference =
            differenceInMinutes(maxDate, event.start) / 60;
